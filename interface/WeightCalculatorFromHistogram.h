...
class WeightCalculatorFromHistogram {
 public:
  ...
  WeightCalculatorFromHistogram(TH1 *hist, TH1* targethist, bool norm=true, bool fixLargeWeights=true, bool verbose=false, bool norm_to_targetArea=false);
  ~WeightCalculatorFromHistogram() {}
 ...
 private:
  std::vector<float> loadVals(TH1 *hist, bool norm=true, bool norm_to_targetArea=false);
  ...
  bool verbose_;
  bool norm_;
  bool norm_to_targetArea_; // for PU weights it could be better to normalize MC to data, rather than both to 1, to avoid ratios of bins with very small numbers
};
